...
const PopoverSidebar = ({ trans }: { trans: string }) => {
  ...
  const { layout, isRtl, menu } = useThemeStore();
  const dynamicMenu = menu.map((el) => ({ title: el.title, href: el.href }));
  const menus = menu || [];
  console.log(menu);
  ...
  return (
    <div
      className={cn("fixed  top-0  border-r  ", {
        "w-[248px]": !collapsed,
        "w-[72px]": collapsed,
        "m-6 bottom-0   bg-card rounded-md": layout === "semi-box",
        "h-full   bg-card ": layout !== "semi-box",
      })}
    >
      {sidebarBg !== "none" && (
        <div
