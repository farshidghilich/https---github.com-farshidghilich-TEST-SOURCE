...
const Language = () => {
  ...
  const [selectedLanguage, setSelectedLanguage] = useState<Language>(found ?? languages[3]);
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button type="button" className="bg-transparent hover:bg-transparent">
          ...
          <span className="text-sm text-default-600 capitalize">{selectedLanguage ? selectedLanguage.name : "En"}</span>
        </Button>
      </DropdownMenuTrigger>
        ...
        {languages.map((item, index) => (
          <DropdownMenuItem
            ...
            className={cn("py-1.5 px-2 cursor-pointer dark:hover:bg-background mb-[2px] last:mb-0", {
              "bg-primary-100 ": selectedLanguage && selectedLanguage.name === item.name,
            })}
          ...
          >
            ...
            <span className="text-sm text-default-600 capitalize">{item.name}</span>
