...
import { Bus, FileCheck, Search } from "lucide-react";
...
const VerticalHeader: React.FC<VerticalHeaderProps> = ({
  ...
  const {showList , list} = useControler<organizationType>('v1/pts/organization')
  console.log(list)
  ...
  const SearchButton = (
    <div className="flex gap-4">
      ...
      >
        <FileCheck className="h-6 w-6  cursor-pointer" />
      </Link>
      <Link
        href="/Freighter"
        className="flex flex-col items-center justify-center"
      >
        <Bus className="h-6 w-6  cursor-pointer" />
      </Link>
      <Select dir="rtl">
      <SelectTrigger>
      ...
      <SelectContent>
        {list.map((item)=>(
        <SelectItem value={String(item.id)}>{item.fullTitle}</SelectItem>
        ))}
      </SelectContent>
    </Select>
    </div>
  );
  if (layout === "semi-box" && !isDesktop) {
