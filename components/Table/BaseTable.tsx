...
import InputMap from "../Map/Map";
import { useForm, SubmitHandler, Controller } from "react-hook-form";
import { zodResolver } from "@hookform/resolvers/zod";
import { z } from "zod";
import Image from "next/image";
import { FiledTypes, propsBaseTable } from "./baseTable.types";
const persianDate = new PersianDate();

function BaseTable<T extends AnyThing>({
  Filed,
  NestedTable,
  BreadCrumbOption,
  Url,
  ...
  //             {fields.map((filetype) => {
  //               const { name, type, accessorKey, placeholder, validation } =
  //                 filetype;

  //               return (
  //                 <div className="flex flex-col gap-2" key={name as string}>
  //                   <label>{accessorKey}</label>
  ...
  //                     defaultValue=""
  //                     rules={{
  //                       required: validation?.required && "این فیلد الزامی است",
  ...
  //                           className="p-2 border rounded-md" // اضافه کردن استایل‌های سفارشی
  //                         />
  //                       );
  //                     }}
  //                   />
  ...
  //           </div>
  //         </div>
  //         <div className="space-x-4 rtl:space-x-reverse fixed bottom-0 bg-card z-10 p-2 text-center pb-4">
  //           <button
  //             type="button"
                      ...
                      return (
                        ...
                        >
                          ...
                          <Input
                            ...
                            {...register(filetype.name as string, {
                              required: filetype.required
                                ? "این فیلد الزامی است"
                                : false,
                              pattern: filetype.pattern
                                ? {
                                    value: filetype?.pattern,
                                    message: filetype?.messagePattern,
                                  }
                                : undefined,
                            })}
                          />
